...
var firstInputLine = firstInput.components(separatedBy: ",")
var secondInputLine = secondInput.components(separatedBy: ",")
...
typealias Coordinate = GridPoint

func findCoordinatesForLine(path: [String]) -> Set<Coordinate> {
    var x = 0
    var y = 0
    var coordinates: Set<Coordinate> = Set<Coordinate>()
    
    for instruction in path {
        let direction = instruction[instruction.startIndex]
        let amount = Int(instruction[instruction.index(instruction.startIndex, offsetBy: 1)..<instruction.endIndex]) ?? 0
        
        switch direction {
        case "L":
            for _ in 0..<amount {
                x -= 1
                coordinates.insert(Coordinate(x: x, y: y))
            }
            break
        case "R":
            for _ in 0..<amount {
...
func findClosestIntersectionForLines(_ firstLine: [String], _ secondLine: [String]) -> Coordinate {
    let firstLineCoordinates = findCoordinatesForLine(path: firstLine)
    var intersections: [Coordinate] = []

    var x = 0
    var y = 0
    
    for instruction in secondLine {
        let direction = instruction[instruction.startIndex]
        let amount = Int(instruction[instruction.index(instruction.startIndex, offsetBy: 1)..<instruction.endIndex]) ?? 0
        ...
        for _ in 0..<amount {
            ...
            if (firstLineCoordinates.contains(Coordinate(x: x, y: y))) {
                intersections.append(Coordinate(x: x, y: y))
            }
        }
    }
    
    return intersections.sorted { (lhs, rhs) -> Bool in
        return manhattanDistance(from: lhs) < manhattanDistance(from: rhs)
    }.first ?? Coordinate(x: 0, y: 0)
}

func manhattanDistance(from coordinate: Coordinate) -> Int {
...
func findManhattanDistanceForClosestIntersectionInLines(_ firstLine: [String], _ secondLine: [String]) -> Int {
    let closestIntersection = findClosestIntersectionForLines(firstLine, secondLine)
    return manhattanDistance(from: closestIntersection)
}

// Example test cases
assert(findClosestIntersectionForLines(["R8","U5","L5","D3"], ["U7","R6","D4","L4"]) == Coordinate(x: 3, y:3))
...
assert(findManhattanDistanceForClosestIntersectionInLines(firstInputLine, secondInputLine) == 399)
