...
func stepThroughLine(path: [String], onEveryStep closure: ((GridPoint) -> Void)) {
    var point = GridPoint(x: 0, y: 0)
    
    for instruction in path {
        let direction = instruction[instruction.startIndex]
        let distance = Int(instruction[instruction.index(instruction.startIndex, offsetBy: 1)..<instruction.endIndex]) ?? 0
...
func findIntersectionsForLines(_ firstLine: [String], _ secondLine: [String]) -> [GridPoint] {
    ...
    stepThroughLine(path: firstLine) { point in
        firstLineGridPoints.insert(point)
    }
    
    var intersections: [GridPoint] = []
    
    stepThroughLine(path: secondLine) { (point) in
        if(firstLineGridPoints.contains(point)) {
...
func manhattanDistance(from GridPoint: GridPoint) -> Int {
    return abs(GridPoint.x) + abs(GridPoint.y)
}
